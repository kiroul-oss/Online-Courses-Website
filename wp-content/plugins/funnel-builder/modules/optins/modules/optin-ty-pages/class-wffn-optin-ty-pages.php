...
if ( ! class_exists( 'WFFN_Optin_TY_Pages' ) ) {
	class WFFN_Optin_TY_Pages extends WFFN_Module_Common {
		...
		public function parse_request_for_oty() {
			...
			if ( ! isset( $data['custom_redirect'] ) || ( 'true' !== $data['custom_redirect'] ) ) {
				return;
			}
			...
			if ( is_null( $custom_page ) || ! $custom_page instanceof WP_Post ) {
				return;
			}

			/** Valid page found, redirect */				
			/* Redirect Only if curent page and custom page id are different */
			if( get_the_ID() !== absint($custom_page->ID) ){
		...
		public function add_elementor_page_templates( $templates ) {
			...
			if ( true === apply_filters( 'wffn_add_existing_templates', false, 'optin_ty' ) && is_array( $templates ) ) {
